...
export const orderSlice = createSlice({
    ...
    reducers: {
        doAddBookAction: (state, action) => {
            ...
            let isExistIndex = carts.findIndex(c => c.bookId === item.bookId);
            if (isExistIndex > -1) {
                ...
                carts.push({ quantity: item.quantity,bookId: item.bookId, detail: item.detail })
            }
            //update redux
            state.carts = carts;
        ...
        doDeleteItemCartAction: (state, action) => {
            state.carts = state.carts.filter(c => c.bookId !== action.payload.bookId);
        },
